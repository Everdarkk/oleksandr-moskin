...
// Описує структуру об'єкта зорі
interface Star {
  x: number;      // 3D координата X
  y: number;      // 3D координата Y
  z: number;      // 3D координата Z (глибина)
  px: number;     // Попередня екранна позиція X
  py: number;     // Попередня екранна позиція Y
  color: string;  // Колір зорі
}

// Описує точку з 2D координатами
interface Point {
  x: number;
  y: number;
}
...
// Головний компонент, що рендерить фон зоряного поля на весь екран
...
// Компонент, що відповідає за логіку та рендеринг зоряного поля
